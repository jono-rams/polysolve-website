      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <section>
            ...
            <p>
              A <code>Function</code> is initialized with its highest degree.
            </p>
            ...
            <CodeBlock language="python">
              {`# f(x) = 4x³ - 2x² + 0x - 9
# v0.7.0 Update: Coefficients can also be complex numbers!
# f(x) = (1+2j)x³ - 5j
my_func.set_coeffs([1+2j, 0, 0, -5j])`}
            </CodeBlock>
          </section>
          <section>
          ...
          <section>
            ...
            <p>
              PolySolve 0.7.0 introduces <code>get_roots()</code>, the primary entry point for finding
...
# Calculate all roots (returns complex128 array)
# roots = [0.+1.j, 0.-1.j]
roots = f.get_roots(options={"find_complex": True})
print(roots)

# You can still use the helper methods for specific needs:
real_roots_only = f.get_real_roots() 
target_roots = f.solve_x(y_val=50) # Find x where f(x) = 50`}
            </CodeBlock>
          </section>
          <section>
            <h3 className="font-semibold">Arithmetic Operations</h3>
            <p>
              <code>Function</code> objects support addition, subtraction, and
              multiplication. These operations now support **complex scalars** and
