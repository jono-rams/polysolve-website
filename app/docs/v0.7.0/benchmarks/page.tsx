...
def run_suite():
    ...
    """
    degrees = [3, 5, 10, 20, 40, 60, 80, 100]
    repeat_count = 10 
    ...
    for deg in degrees:
        ...
        for i in range(repeat_count):
            ...
            np_maes.append(calculate_mae(coeffs, np_roots))

            # --- 2. Test PolySolve (CPU) ---
            start_ps_cpu = time.perf_counter()
            ps_cpu_roots = ps_poly.get_roots(options=ga_options, use_cuda=False) 
            ps_cpu_times.append(time.perf_counter() - start_ps_cpu)
            ps_cpu_maes.append(calculate_mae(coeffs, ps_cpu_roots))
            
            # --- 3. Test PolySolve (GPU) ---
            if _CUPY_AVAILABLE:
                start_ps_gpu = time.perf_counter()
                ps_gpu_roots = ps_poly.get_roots(options=ga_options, use_cuda=True)
                ps_gpu_times.append(time.perf_counter() - start_ps_gpu)
                ps_gpu_maes.append(calculate_mae(coeffs, ps_gpu_roots))
            else:
    ...
    <div className="space-y-8 prose prose-invert max-w-none">
      ...
      <div className="space-y-4">
        <p>
          ...
          PolySolve&lsquo;s stability and performance against NumPy. The full
          Python script used to generate these results is provided below.
        </p>
        <p>This script was designed to test two key metrics:</p>
        <ul>
          <li>
            <strong>Accuracy:</strong> It measures the Mean Absolute Error (MAE)
            between the known roots of a generated polynomial and the roots
            found by each library.
          </li>
          <li>
            <strong>Performance:</strong> It measures the wall-clock time taken
            to solve polynomials of increasing degree.
          </li>
        </ul>
      </div>

      <h2 className="text-2xl font-bold">Benchmark Script</h2>
      <CodeBlock language="python">{benchmarkScript}</CodeBlock>
